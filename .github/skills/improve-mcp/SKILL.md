---
name: improve-mcp
description: 'Improve the EasyEDA MCP service through live end-to-end validation. Use when fixing bridge or server reliability, testing MCP tool behavior in EasyEDA Pro, creating a disposable project/schematic/PCB from scratch, reconnecting the EasyEDA bridge, or iterating until PCB creation is as complete as possible and Gerber export is reachable.'
...
- Exercising the full project -> schematic -> PCB workflow through MCP.
...
- Chrome DevTools MCP for opening EasyEDA Pro, confirming page state, checking login/editor readiness, inspecting progress toward a complete PCB, reading console errors, inspecting network activity, taking snapshots or screenshots, and interacting with the EasyEDA design UI when necessary, such as reconnecting the MCP bridge.
...
- The public EasyEDA documentation at `https://docs.easyeda.com/en/` for extra host-product and workflow context when the live runtime or repo code leaves behavior ambiguous.
...
1. Open EasyEDA Pro with Chrome DevTools MCP and confirm the editor is fully loaded.
...
5. Exercise the relevant MCP tools by placing parts, wiring or connecting nets, saving documents, and pushing the workflow as far as possible through MCP.
6. Measure progress by how far the MCP server can drive the workflow toward a fully complete PCB, not by partial success on isolated tool calls.
7. Treat any mismatch between advertised tool behavior and observed runtime behavior as a service bug in this repository.
8. Append an iteration entry to the report capturing the failure symptoms, exact errors, browser or bridge observations, and the code area suspected.
9. Fix the root cause in the bridge or server code.
10. Append the code changes made in that iteration to the report, including touched files and the intended effect of the change.
11. When restarting, detect and terminate stale local `mcp:server` processes first, then start exactly one fresh MCP server instance.
12. Reconnect the EasyEDA bridge automatically when required by the change or by bridge instability, using Chrome DevTools MCP to operate the EasyEDA UI when necessary.
13. Verify the restarted system with `bridge_status` and context checks before continuing.
14. Before treating PCB work as successful, verify that the schematic components are mapped to the intended PCB footprints and that no footprint mismatch or invalid-footprint issue remains.
15. Before DRC or fabrication-export validation, verify that the PCB has explicit board-outline geometry on the board outline layer and that the intended board size is therefore defined.
16. Before treating routing as complete, verify that no intended connections remain as unrouted ratlines or other unresolved net connectivity in the PCB view.
17. Before treating PCB work as successful or proceeding to export-oriented PCB steps, run the relevant EasyEDA DRC flow and resolve every reported error and warning unless a warning is a verified host-side false positive that is documented explicitly in the report.
18. When fabrication output is part of the goal, verify that the Gerber export flow is reachable, and when practical also verify the generated Gerber/Drill output in a viewer or EasyEDA's Gerber view before treating the board as fabrication-ready.
19. When assembly output is part of the goal, verify that BOM and Pick and Place export paths are reachable and note any missing designators, footprint metadata, or placement-origin issues that would make assembly output incomplete.
20. Rerun the same end-to-end workflow, then append the validation outcome, remaining gap, and next hypothesis to the report.
21. Continue iterating until the PCB flow completes successfully through MCP or a genuine external blocker remains.
...
2. Use Chrome DevTools MCP to inspect the EasyEDA page for console errors, page state, modal dialogs, stale UI context, and visible PCB-design progress.
...
5. Only conclude there is an external blocker after the above steps fail.
